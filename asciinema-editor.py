...
class Recording(object):
    ...
    def limit_idle_time(self, idle_time_limit):
        last_time_old = 0
        last_time_new = 0
        for line in self.lines:
...
def main():
    ...
    if op == "add-offset":
        ...
        r = Recording.from_file(input_filename)
        r.limit_idle_time(idle_time_limit)
        r.write(output_filename)
    elif op == "-h":
        print("asciinema video editor")
        print("Usage:")
        print("  {} add-offset <input file> <output file> <offset>".format(sys.argv[0]))
        print("  {} limit-idle-time <input file> <output file> <idle time limit>".format(sys.argv[0]))
    else:
